...
// app.get('*', async (ctx) => {
//   console.log('Got request', new URL(ctx.req.url).href)
//   const countInput = Number(ctx.req.query('count'))
//   const parallel = ctx.req.query('parallel') === 'true' ? true : false
//   const count = isNaN(countInput) || countInput <= 0 ? 100 : countInput
//   const now = new Date()
//   console.time('loadComponents')
//   const components = await loadJsonFile<string[]>(`./components.json`)
//   if (!components) {
//     return ctx.text('Components not found', { status: 404 })
//   }
//   const componentFileTime = new Date().getTime() - now.getTime()
//   console.timeEnd('loadComponents')
//   const loadComponentsStart = new Date()
//   const componentLoadTimes: Record<string, number> = {}
//   console.time('load each component')
//   if (!parallel) {
//     for (let i = 0; i < Math.min(count, components.length); i++) {
//       const componentName = components[i]
//       const componentLoadStart = new Date()
//       console.time(`load ${componentName}`)
//       const component = await loadJsonFile(`./components/${componentName}.json`)
//       console.timeEnd(`load ${componentName}`)
//       if (!component) {
//         break
//       }
//       const componentLoadEnd = new Date()
//       componentLoadTimes[componentName] =
//         componentLoadEnd.getTime() - componentLoadStart.getTime()
//     }
//   } else {
//     await Promise.all(
//       components
//         .slice(0, Math.min(count, components.length))
//         .map(async (name) => {
//           const componentLoadStart = new Date()
//           await loadJsonFile(`./components/${name}.json`)
//           const componentLoadEnd = new Date()
//           componentLoadTimes[name] =
//             componentLoadEnd.getTime() - componentLoadStart.getTime()
//         }),
//     )
//   }

//   console.timeEnd('load each component')
//   const loadComponentsEnd = new Date()
//   return ctx.json({
//     parallel,
//     componentFileTime,
//     loadComponentsTime:
//       loadComponentsEnd.getTime() - loadComponentsStart.getTime(),
//     componentLoadTimes,
//   })
// }) // TODO: remove this
app.get(
